import { PipeTransform, ArgumentMetadata, BadRequestException, HttpException } from '@nestjs/common';
...
export class fileValidationPipe implements PipeTransform {
    async transform(value: Express.Multer.File, metadata: ArgumentMetadata) {
        ...
        await sharp(value.buffer)
            .resize(500,500).toFormat('jpeg').jpeg({quality:80}).toFile(`src/uploads/brand/${filename}`);
        return filename;
    };
};